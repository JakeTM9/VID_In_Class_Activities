  ...
  .then(data => {  //ARROW function - pass data as a parameter into this anonymous function
    ...
    data.forEach(d => { //ARROW function - for each object in the array, pass it as a parameter to this function
		...
		let tokens = d.start.split("-");
  		...
  		let cx = 0; //******* don't use 0, use mapFromDomainToRange. Hint- use 365 days and don't worry about leap years... it is ok
  		let cy = 0; //******* don't use 0, use getYearPosition
  		let radius = 10; //******* don't use 10, use mapFromDomainToRange.  The max cost is 160, and the min cost is 1
  		let color = "gray"; //******* don't use gray, use lookupColor
		//rangx1 = timeLineX1
		//rangx2 = timeLineX2
		//d1 = 0
		//d2 = 365
		//v = daysFromStart
		cx = mapFromDomainToRange(0,365,timelineX1,timelineX2,d.daysFromYrStart);
		cy = getYearPosition(year,gapBetweenTimelines,startOfTimelinesY);
		radius = mapFromDomainToRange(timelineX1,timelineX2,1,160,d.daysFromYrStart)
     	// for each event, draw a circle, along the timeline
		color = lookupColor(d.category);
